"""Unit tests for queen.queen (S4.1)."""
...
def test_evaluate_turn_parses_sufficient():
    ...
    label = queen.evaluate_turn(_record(1))
    assert label.turn == 1
...
def test_evaluate_turn_parses_insufficient():
    ...
    label = queen.evaluate_turn(_record(2))
...
def test_run_batch_respects_sampling_rate():
    ...
    labels = queen.run_batch(log, sample_rate=0.1)  # every 10th turn
...
def test_run_batch_sample_rate_50():
    ...
    labels = queen.run_batch([_record(i) for i in range(6)], sample_rate=0.5)
...
def test_malformed_json_raises():
    ...
    with pytest.raises(Exception):
        queen.evaluate_turn(_record(1))


def test_empty_response_raises():
    queen = Queen(generate_fn=lambda p: "")
    import pytest
