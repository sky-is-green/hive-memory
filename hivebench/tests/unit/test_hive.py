...
def test_enable_medium_wires_real_medium_drone():
    ...
    config = HiveConfig(enable_medium=True, medium_model="microsoft/graphcodebert-base")
    h = Hive(config=config, ultra=FakeUltraSmall(), backend=None)
...
def test_enable_medium_false_uses_placeholder():
    config = HiveConfig(enable_medium=False)
    h = Hive(config=config, ultra=FakeUltraSmall(), backend=None)
