...
JSON/CSV files are also the review surface for human or LLM-oracle annotation:
override any ``*_auto`` field with a reviewed label.

Usage::

    python -m oracle.labeling --output tests/fixtures/labels
"""
...
DEFAULT_LABEL_DIR = Path(__file__).resolve().parents[1] / "tests" / "fixtures" / "labels"
...
def generate_routing_decision_labels(conversations, n: int = 200) -> list[dict]:
    ...
    proxy oracle until a real oracle labels these (see note in the record)."""
