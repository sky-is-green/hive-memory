...
Stores oracle labels and hive decisions for retrieval-quality metrics:
...
class GroundTruthDB:
    ...
    def record_oracle_label(
        ...
        self._conn.execute(
            "INSERT INTO oracle_labels(turn, chunk_id, predicted_relevant, actually_relevant, score) "
            "VALUES (?,?,?,?,?)",
    ...
    def record_routing_decision(self, turn: int, score: float, optimal_route: str) -> None:
        """Record a routing decision with its heuristic score and the oracle's
    ...
    def _label_rows(self, window: int) -> list:
        return self._conn.execute(
            ...
            "(SELECT * FROM oracle_labels ORDER BY id DESC LIMIT ?)",
            (window,),
        ).fetchall()
    ...
    def label_count(self) -> int:
        return self._conn.execute("SELECT COUNT(*) AS n FROM oracle_labels").fetchone()["n"]
