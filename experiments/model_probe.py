...
def probe_model(
    ...
    ``http`` is injectable for tests and must expose ``get(url, timeout=)`` and
...
def run_probe(
    ...
    try:
        for i, model in enumerate(models, 1):
            print(f"[{i}/{len(models)}] probing {model} ...", flush=True)
            r = probe_model(
                base_url, model, max_tokens=max_tokens,
                disable_thinking=disable_thinking, http=http,
            )
            results.append(r)
    finally:
        if ka is not None:
