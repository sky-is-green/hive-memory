"""S0.5 baseline 1: LM Studio (Qwen3.6) with default rolling context, no hive.

Usage::

    python -m cortex.baselines.lm_studio_baseline [--conversations DIR] [--mock]

Records metrics to ``logs/baseline_lmstudio.json``. Pass ``--mock`` to verify the
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="LM Studio rolling-context baseline")
    parser.add_argument(
        "--conversations",
    ...
    parser.add_argument(
        "--output", default="logs/baseline_lmstudio.json", help="output JSON path"
    )
    parser.add_argument(
    ...
    if args.mock:
        ...
        client = __import__(
            "cortex.baselines.lm_studio_client", fromlist=["LMStudioClient"]
        ).LMStudioClient(base_url=args.base_url, model=args.model)
    ...
    results = runner.run_baseline(
        ...
        client,
        mode="lm_studio",
        build_messages=runner.build_lmstudio_messages,
