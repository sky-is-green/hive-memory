"""Resilience tests (E1): malformed responses, unreachable backend, oracle."""
...
def test_oracle_malformed_json_raises():
    oracle = AsyncOracle(generate_fn=lambda p: "not json")
    with pytest.raises(Exception):
        oracle.evaluate_turn(TurnRecord(1, "c", "q", "r"))


def test_oracle_valid_json_ok():
    oracle = AsyncOracle(
        generate_fn=lambda p: json.dumps({"sufficient": True, "used_pieces": [], "missing": [], "score": 4})
    )
    label = oracle.evaluate_turn(TurnRecord(1, "c", "q", "r"))
