"""Unit tests for oracle.ground_truth (S4.2)."""
...
def test_precision_recall_false_eviction_hand_computed(db):
    ...
    db.record_oracle_label(1, "A", True, True)
    db.record_oracle_label(1, "B", True, False)
    db.record_oracle_label(1, "C", False, True)
    db.record_oracle_label(1, "D", False, False)
...
def test_window_limits_queries(db):
    for _ in range(50):
        db.record_oracle_label(1, "x", True, True)   # all correct
    for _ in range(50):
        db.record_oracle_label(1, "x", False, False)  # all evicted-correctly
...
def test_10000_labels_performant(db):
    ...
    for i in range(10000):
        db.record_oracle_label(i % 100, "c", bool(i % 2), bool(i % 3))
