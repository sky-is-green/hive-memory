...
def decay_db():
    ...
    for _ in range(4):
        db.record_oracle_label(turn=1, chunk_id="old_rel", predicted_relevant=True,
                               actually_relevant=True, score=0.9)
        db.record_oracle_label(turn=1, chunk_id="old_irr", predicted_relevant=False,
                               actually_relevant=False, score=0.2)
        db.record_oracle_label(turn=10, chunk_id="recent", predicted_relevant=True,
                               actually_relevant=True, score=0.9)
    yield db
    db.close()
...
def test_optimize_routing_replay_finds_interior_optimum(routing_db):
    ...
    # threshold 2 matches the oracle's own default and minimizes cost -> best
    assert best == 2
