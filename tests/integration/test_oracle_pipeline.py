"""Integration: S4 oracle roundtrip and A/B testing with the real assembler."""
...
def test_oracle_to_ground_truth_roundtrip():
    assembler = ContextAssembler()
    store = _store()
    assembled = assembler.assemble(
    ...
    for cid in assembled.selected_chunk_ids:
        # oracle says every selected chunk was relevant
        db.record_oracle_label(1, cid, True, bool(label.chunk_labels.get(cid, True)))
